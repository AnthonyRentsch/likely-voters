...
# run model
formula <- as.formula(as.factor(validated) ~ as.factor(vote12) + 
                        as.factor(intent) + 
                        as.factor(interest) + 
...
train <- train[train$intent < 8,]
train <- train[train$interest < 8,]
train <- train[train$income != 98,]
train <- train[!train$partisanship %in% c(8, 98, 99),]
train <- train[!train$religiosity %in% c(8,9) & !is.na(train$religiosity),]
...
test <- test[test$intent < 8,]
test <- test[test$interest < 8,]
test <- test[test$income != 98,]
test <- test[!test$partisanship %in% c(8, 98, 99),]
test <- test[!test$religiosity %in% c(8,9) & !is.na(test$religiosity),]
...
formula <- as.formula(as.factor(validated) ~ as.factor(vote12) + 
                        ...
                        as.factor(religiosity) +
                        as.factor(marital_status) +
                        residential_mobility +
                        as.factor(watch_news) +
                        as.factor(read_paper))
model <- glm(formula, data = train, family = "binomial")
