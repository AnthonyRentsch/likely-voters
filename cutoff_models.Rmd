...
# needs recoding faminc, pid7, CC16_300b, CC16_300c, CC16_300d_1, CC16_300d_2, CC16_300d_3, CC16_300d_4, CC16_300d_5
# may need to recode some of these in load.R
# to see how these variables are coded, see load.R
data <- cces16 %>% select(V101, state_abbreviation, CC16_364b, CC16_364c,  
  ...
  rename(ID = V101, state = state_abbreviation, earlychoice = CC16_364b, choice = CC16_364c, 
         ...
         posted_story_sm = CC16_300d_1, comment_sm = CC16_300d_2, read_story_sm = CC16_300d_3,
         followed_event_sm = CC16_300d_4, forwarded_story_sm = CC16_300d_5) %>% 
  mutate(age = 2016 - birthyr) %>% 
  select(-birthyr)
...
# run model
formula <- as.formula(as.factor(validated) ~ as.factor(vote12) + as.factor(intent) + as.factor(interest))
...
margin_by_turnout %>% 
  ...
  ggplot() +
  geom_smooth(aes(x = turnout_rates[i]*100, y = margin), alpha = 0.75, se = FALSE) +
...
Note that this list should not be viewed as comprehensive - it reflects the research that I conducted for this project and it is possible that I have missed potentially important variables.
...
# run model
formula <- as.formula(validated ~ vote12 + intent + interest + age) #ADD MORE!!!!!!!!!
model <- glm(formula, data = train, family = "binomial")
