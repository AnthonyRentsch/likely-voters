...
df %>% gather(type, value, true_positive:true_negative) %>%
  ...
  labs(x = "model", y = "", title = "Individual level vote validation by vote intent") +
  scale_fill_grey() +
...
df %>%  
  ...
  coord_flip() +
  geom_hline(yintercept = 4.4, lty = 2, colour = "red") +
...
validation_intent_history <- function(intention, history){
  willvote <- pooled %>% 
    filter(intent %in% intention, vote_history %in% history 
           #| age <= 22
           )
  wontvote <- pooled %>% 
    anti_join(willvote, by = "case_id")
...
# vote intent
pooled$perry_gallup[!is.na(pooled$intent) & (pooled$intent == 1 | pooled$intent == 3)] <- pooled$perry_gallup + 2
pooled$perry_gallup[!is.na(pooled$intent) & pooled$intent == 2]  <- pooled$perry_gallup + 1

# vote history
pooled$perry_gallup[!is.na(pooled$vote_history) & pooled$vote_history == 4] <- pooled$perry_gallup + 1

# political interest
pooled$perry_gallup[!is.na(pooled$interest) & pooled$interest == 1] <- pooled$perry_gallup + 2
pooled$perry_gallup[!is.na(pooled$interest) & pooled$interest == 2] <- pooled$perry_gallup + 1

# voter registration 
pooled$perry_gallup[pooled$registration == 1] <- pooled$perry_gallup + 1

# age adjustment
pooled$perry_gallup[pooled$age < 22 && pooled$vote_history == 0] <- pooled$perry_gallup + 1
```
...
formula <- as.formula(validated ~ vote_history + intent + interest + registration + eligible)
...
predictions_unweighted <- predictions_unweighted %>% 
  rename(response = 'predict(model_unweighted, newdata = test, type = \"response\")')
# weighted
...
ggplot(predictions_unweighted) +
  ...
  labs(title = paste0("Predicted vote propensity scores for 2016 CCES holdout sample (N = ",
                      nrow(predictions_unweighted), ")", sep = ""),
...
validation_by_logreg <- function(turnout){
  willvote <- predictions_unweighted %>% 
    arrange(desc(response)) %>% 
    top_n(round(nrow(predictions_unweighted)*turnout,0))
  wontvote <- predictions_unweighted %>% 
    anti_join(willvote, by = "case_id")
...
vote_choice_logreg <- function(turnout){
  ...
  top_n(round(nrow(predictions_unweighted)*turnout,0), wt = response) %>% 
  filter(choice %in% c("Donald Trump (Republican)","Hillary Clinton (Democrat)",
                       "Gary Johnson (Libertarian)","Jill Stein (Green)","Other",
                       "I'm Not Sure")) %>% 
  group_by(choice) %>% 
...
margin_by_turnout %>% 
  ...
  geom_hline(yintercept = 4.4, lty = 2, col = "red") +
  #coord_cartesian(xlim = c(25, 100)) +
  labs(title = paste0("Clinton's predicted margin of victory computed from holdout sample (N = ",
                      nrow(predictions_unweighted), ")", sep = ""),
       x = "Turnout (%)",
