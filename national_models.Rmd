...
true_pos_neg_rates_by_turnout %>% 
  ...
  labs(title = "True positive and true negative rates for 2016 CCES sample",
       x = "Turnout rate (%)",
       y = "",
       lty = "Type") +
    theme(legend.title=element_blank())
...
ggplot() +
  geom_point(data = margin_by_turnout, 
             aes(x = turnout_rates[i]*100, y = margin, colour = "unweighted"), alpha = 0.5) +
  geom_point(data = weighted_margin_by_turnout, 
             aes(x = turnout_rates[i]*100, y = margin, colour = "weighted"), alpha = 0.5) +
  geom_hline(yintercept = 4.4, lty = 2, col = "red") +
  #coord_cartesian(xlim = c(25, 100)) +
  labs(title = paste0("Clinton's predicted margin of victory computed from 2016 CCES sample (N = ",
                      nrow(predictions), ")", sep = ""),
       x = "Turnout (%)",
