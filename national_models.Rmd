...
fig1 <- df %>%  
  ...
  coord_flip() +
  geom_hline(yintercept = validated_margin16, lty = 2, colour = "red") +
...
validation_intent_history <- function(intention, history){
  willvote <- pooled %>% 
    filter(year == 2016, intent %in% intention, vote_history %in% history 
           # age <= 22
           )
  wontvote <- pooled %>% filter(year == 2016) %>% 
    anti_join(willvote, by = "case_id")
...
fig2 <- df_combined %>% gather(`Vote history`, value, `Voted in 2012`:All) %>%  
