...
validation_by_logreg_state <- function(location, turnout){
  willvote <- predictions_all %>% filter(state == location) %>% 
    top_n(round(nrow(predictions_all)*turnout,0), wt = response)
...
ggplot() +
  ...
  labs(title = paste0("Error between predicted margin and actual margin, \n computed from 2016 CCES sample (N = ", nrow(predictions_all), ")", sep = ""),
       x = "Turnout (%)",
...
mean_weighted_errors <- margin_by_turnout_all %>% group_by(i) %>% 
  summarise(avg_error = mean(error_weighted))

ggplot() +
  geom_point(data = margin_by_turnout_all, 
             aes(x = turnout_rates[i]*100, y = error_weighted), colour = "grey", alpha = 0.25) +
  geom_line(data = mean_weighted_errors, 
             aes(x = turnout_rates[i]*100, y = avg_error), colour = "blue", size = 1) +
  geom_hline(yintercept = 0, lty = 2, col = "black") +
  labs(title = paste0("Error between predicted margin and weighted margin (full sample), \n computed from 2016 CCES sample (N = ", nrow(predictions_all), ")", sep = ""),
       x = "Turnout (%)",
       y = "Error between predicted and weighted margin") +
  theme(plot.title = element_text(hjust = 0.5))
```
